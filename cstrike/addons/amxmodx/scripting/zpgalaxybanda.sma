...
new const gArmasOptimizadas[ARMITAS][MAX_ATRIBUTOS] = {
	...
	{ "Coneja Special (Da̱o x5)", 0, 0, 0, 255, 61, 216 , 20, 5.0, 158, 0, CSW_SCOUT, "weapon_scout", "models/zpre4/armas/v_pink_scout.mdl", "default", INACTIVE },
...
new const gHabilidadesHumanas[MAX_HAB][Habilidades_Hm] = 
{
	...
	{ "Aplicado", "Hace + da̱o x Headshot", 30, 0 },
...
public fw_PlayerPreThink(id)
{
	...
	if(g_depre[id])
	{
		...
		{			
			...
			{
				zp_colored_print(id, "%s Tienes Que Esperar^x03 %.1f^x01 Para Volver A Utilizar Tu Ca̱on", TAG, get_pcvar_float(cvar_depredador_cooldown)-(get_gametime() - gDisparoAnterior[id]))
				return;
			}
...
public MainMenu(id)
{
	...
	switch(g_Estado[id])
	{
		...
		case Logueado:
		{
			...
			formatex(szData, charsmax(szData), "^n\r(1) \wDesconectarse^n\r(4)\w Cambiar contrase̱a^n\r(6)\w TOP 15^n^n^n\r(0)\w Salir")
		}
	}
...
public Login(id)
{
	...
	{	
		zp_colored_print(id, "%s Contrase̱a no valida, intente nuevamente.", TAG)
...
public Register(id)
{
	...
	{
		zp_colored_print(id, "%sLo sentimos, pero la contrase̱a debe tener al menos %d caracteres!", TAG, get_pcvar_num(gCvarsPlugin[CUENTAS_PASSCHAR][gCvarValor]))
...
public ChangePasswordNew(id)
{
	...
	{
		zp_colored_print(id, "%s^x03Lo sentimos, pero la contrase̱a debe tener al menos^x04 %d^x03 letras!", TAG, get_pcvar_num(gCvarsPlugin[CUENTAS_PASSCHAR][gCvarValor]))
		client_cmd(id, "messagemode NUEVA_PASSWORD")
		return PLUGIN_HANDLED
	}
...
public ChangePasswordOld(id)
{
	...
	{
		zp_colored_print(id, "%sContrase̱a no valida intente nuevamente.", TAG)
...
public ConfirmPassword(id)
{
	...
	formatex(Menu, charsmax(Menu) - length, "\wConfirmar Contrase̱a:... \r %s ^n ^n \r1 \w Confirmar ^n \r2 \w Volver a escribir la contrase̱a ^n ^n ^n \r0 \w Cancelar", new_pass[id])
	...
	switch(key)
	{
		case 0:
		{
			...
			if(g_Estado[id] >= Logueado)
			{
				...
				password[id] = hash
				zp_colored_print(id, "%sHaz cambiado correctamente la contrase̱a! Nueva contrase̱a:^x04 %s", TAG, new_pass[id])

				if(get_pcvar_num(gCvarsPlugin[CUENTAS_LOGCAMBIARPASS][gCvarValor]))
				{
					log_to_file(log_file, "%s ha cambiado su contrase̱a", name)
				}
